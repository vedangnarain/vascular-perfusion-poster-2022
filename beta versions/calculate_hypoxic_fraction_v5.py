...
Version 5: Compute for stochastic simulations and highlight experimental 
hypoxic thresholds.
...
def get_hypoxic_fraction(solver_name, alpha_value, beta, trial, hypoxic_threshold, plot=0):    
...
# Define function to compute the average HT of all trials in a beta value
def compute_average_trial(solver_name, alpha_value, beta, hypoxic_threshold, max_trials):
    ...
    for trial in range(1, max_trials):    
        hypoxic_fraction, mean_value, min_value, half_value, max_value, std_value = get_hypoxic_fraction(solver_name, alpha_value, beta, str(trial), hypoxic_threshold, plot=0)
    ...
    return np.average(beta_table[:,0]), np.average(beta_table[:,1]), np.average(beta_table[:,2]), np.average(beta_table[:,3]), np.average(beta_table[:,4]), np.average(beta_table[:,5])

# Define a function to return the hypoxic fraction data for all the simulations in a solver
def get_hypoxic_fraction_stats(solver_name, alpha_list, beta_list, max_trials, hypoxic_threshold):
    table = np.array([])
    for alpha_value in alpha_list:
        alpha_table = np.array([])
        for beta in beta_list:
            hypoxic_fraction, mean_value, min_value, half_value, max_value, std_value = compute_average_trial(solver_name, alpha_value, beta, hypoxic_threshold, max_trials)
            table_entry = np.array([float(alpha_value), float(beta), hypoxic_fraction, mean_value, min_value, half_value, max_value, std_value])
...
for i in range(len(hypoxic_threshold_list)):
    solver_data = get_hypoxic_fraction_stats(solver_name, alpha_list, beta_list, max_trials, hypoxic_threshold_list[i])
