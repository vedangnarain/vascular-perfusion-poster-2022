...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  3 14:40:23 2021
...
Calculate some basic distribution stats, the perfusion quotient, and the hypoxic fraction.
...
def get_distribution_stats(solver_name, layout_selection, nhet, kills, hypoxic_threshold_list, plot=0, read=0):    
...
#    folder_path = '/home/narain/Desktop/Stochastic Pruning with 100 Trials/' + solver_name + 'Haematocrit/Lambda4/Alpha' + alpha_value + '/Beta' + beta + '/Trial' + trial
    folder_path = '/home/narain/Desktop/Results/Hexagonal/Biradial/TestHexagonalNetwork/' + solver_name + 'Haematocrit/Selection' + layout_selection + '/NHet' + nhet + '/Kills' + kills
...
#        np.save('/home/narain/Temporary Python Files/' + solver_name + 'Haematocrit_Selection' + layout_selection + '_NHet' + nhet + '_Kills' + kills + '_distribution.npy', flat_field)
#    else:
#        flat_field = np.load('/home/narain/Temporary Python Files/' + solver_name + 'Haematocrit_Selection' + layout_selection + '_NHet' + nhet + '_Kills' + kills + '_distribution.npy')
...
#hypoxic_threshold_list = [0, 20000, 21000, 22000, 23000, 24000] 
...
# Save array
#np.save('/home/narain/Desktop/Results/Hexagonal/Biradial/TestHexagonalNetwork/' + solver_name + 'Haematocrit/python_solver_data.npy', solver_stats)
solver_stats = np.load('/home/narain/Desktop/Results/Hexagonal/Biradial/TestHexagonalNetwork/' + solver_name + 'Haematocrit/python_solver_data.npy')
...
def get_alpha_line(alpha_group, max_beta):
    ...
    pq_table = np.array([])
    alpha_threshold_grouped = alpha_group.groupby(alpha_group.kills)
    for kill in range(1, max_kills+1):
        pq_table_entry = np.array([])
        alpha_threshold_group = alpha_threshold_grouped.get_group(kill)
        pq_table_entry = np.array([alpha_threshold_group["alpha"].mean(), alpha_threshold_group["kills"].mean(), alpha_threshold_group["PQ"].mean()])
...
# Read PQ file
filename = '/home/narain/Desktop/Results/Hexagonal/Biradial/TestHexagonalNetwork/hexagonal_perfusion_quotients.txt'
...
line_0 = get_alpha_line(alpha_0, max_kills)
...
#    axs[i].set_xlabel('vessels killed')  
    ...
    if i==0:
        ...
        axs[i].legend(loc="lower left", prop={'size': 15})        
    axs[i].set_xlim(0)
#    axs[i].legend()
    axs[i].grid()
    axs[i].title.set_text('$P_{thin}$ = ' + heterogeneities[i])

# Plot the PQ for a solver
for i in range(len(alpha_list),len(alpha_list)*2):
    axs[i].set_ylim([0,1.1])  # set PQ limits
#z    axs[i].plot(alpha_0['kills'], pq_composite[i-len(alpha_list)], label='PQ')
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_biradial_hexagonal_individual_pruning.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_biradial_hexagonal_individual_pruning.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
