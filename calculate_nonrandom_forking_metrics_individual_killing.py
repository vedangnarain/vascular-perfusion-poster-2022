...
Calculate metrics of Nonrandom Forking Network with individual vessel pruning.
...
def get_distribution_stats(solver_name, alpha_value, kills, hypoxic_threshold_list, plot=0, read=0):    
    
    # Set the file path
    folder_path = '/home/narain/Desktop/Results/Forking/Nonrandom/Individual Pruning in Nonrandom Forking Network/TestDichotomousNetwork/' + solver_name + 'Haematocrit/Lambda4/Alpha' + alpha_value + '/Kills' + kills
...
def get_solver_stats(solver_name, alpha_list, kills_list, hypoxic_threshold_list, plot, read):
#    table = np.array([])
    alpha_table = np.array([])
    for alpha_value in alpha_list:
        for kills in kills_list:    
            average_kills_data = get_distribution_stats(solver_name, alpha_value, kills, hypoxic_threshold_list, plot, read)
            table_entry = np.hstack([float(alpha_value), float(kills), average_kills_data])
...
solver_stats = get_solver_stats(solver_name, alpha_list, kills_list, hypoxic_threshold_list, plot=0, read=0)
...
# Read PQ file
#filename = '/home/narain/Desktop/Results/Forking/Nonrandom/Radius Threshold Pruning in Nonrandom Forking Network/TestDichotomousNetwork/forking_kills_perfusion_quotients.txt'
filename = '/home/narain/Desktop/Results/Forking/Nonrandom/Individual Pruning in Nonrandom Forking Network/TestDichotomousNetwork/forking_nonrandom_individual_pruning_perfusion_quotients.txt'
pq_df = pd.read_csv(filename, delim_whitespace=True, names=["network_name", "solver_name", "lambda", "alpha", "kills",  "PQ"])
...
for i in range(len(alpha_list*2),len(alpha_list)*3):
    ...
    unpacked_hypoxic_thresholds = np.concatenate( hypoxic_fraction_composite[offset*len(kills_list):(offset+1)*len(kills_list)], axis=0 )
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_individual_pruning.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_individual_pruning.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
